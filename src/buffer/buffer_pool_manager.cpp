...
std::optional<ReadPageGuard> BufferPoolManager::TryReadPage(page_id_t page_id)
{
	...
	std::shared_ptr<FrameHeader> frame = frames_m[page_map_m[page_id]];
	if (not frame->mut.try_lock_shared())
	{
		return std::nullopt;
	}
	std::shared_lock<std::shared_mutex> frame_lk(frame->mut, std::adopt_lock);
	return std::make_optional<ReadPageGuard>(std::shared_ptr<BufferPoolManager>(this), frame, std::move(frame_lk));
}
...
ReadPageGuard BufferPoolManager::WaitReadPage(page_id_t page_id)
{
	...
	frame_lk.lock();
	return ReadPageGuard(std::shared_ptr<BufferPoolManager>(this), frame, std::move(frame_lk));
}
...
std::optional<WritePageGuard> BufferPoolManager::TryWritePage(page_id_t page_id)
{
	...
	return std::make_optional<WritePageGuard>(std::shared_ptr<BufferPoolManager>(this), frame, std::move(frame_lk));
}
...
WritePageGuard BufferPoolManager::WaitWritePage(page_id_t page_id)
{
	...
	frame_lk.lock();
	return WritePageGuard(std::shared_ptr<BufferPoolManager>(this), frame, std::move(frame_lk));
}
...
bool BufferPoolManager::LoadPage(page_id_t page_id)
{
	...
	FrameHeader &frame = *frames_m[*frame_id_opt];
	if (frame.is_dirty)
	{
...
void BufferPoolManager::AddAccessor(frame_id_t frame_id, bool is_writer)
{
	...
	FrameHeader& frame = *frames_m[frame_id];
	frame.pin_count++;
